...
讨论按问题分配给其他席位，新输入只发送分配的问题、相关回应和首评摘要，避免重复嵌入整份问题清单。每次授权的席位、主持、编辑和复核调用使用独立的持久化 DSH 原生会话。Host 提供冻结材料和按稳定席位 ID 组织的最新问题状态；完整原始记录继续保留，但旧轮次不重复进入后续调用。目标和硬约束固定在系统上下文中。角色的 pre-step 过滤其他插件的技能目录和上下文注入，只保留 Host 提交材料与受 DSH 策略约束的工具结果。单次请求仍可由 DSH 压缩，但连续压缩没有有效缩短输入时终止。
