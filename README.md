...
- 15 分钟、1 小时、4 小时和“持续评审”（最长 8 小时）四种固定评审模式；各模式均允许最多 80 轮讨论、400 次模型调用、100 次 MCP 调用，时间与 Token 预算仍按模式区分，达到任一边界即停止派发。界面不暴露底层参数。持续评审适合长时间无人值守运行；观点提前收敛时会提前完成，不会人为等待满 8 小时。
