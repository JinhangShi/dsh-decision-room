...
```sh
dsh plugin --profile web add dsh-decision-room@0.9.2
```
...
- Host 按问题覆盖、证据状态和信息增量决定收尾；连续三轮没有新增问题、改票或证据时强制收敛。票数呈现分歧但不能把事实投票为真。时间是上限，不是必须耗满的时长。
...
席位返回的 `continueDiscussion` 只作为参考。达到独立覆盖后，如果连续三轮没有新增问题、没有改票且没有新证据，Host 会强制进入修订与独立复核；任一轮取得新的 MCP 证据会重新计算这个稳定窗口。MCP 调用的等待授权、执行、拒绝、失败和取消状态都会持久化，宿主重启不会把旧请求误显示为仍在运行。
