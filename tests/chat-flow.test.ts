...
describe("主聊天驱动决策", () => {
  ...
  it("每轮讨论完成后在主聊天追加不可变的 Host 表决快照", async () => {
    ...
    const interpretations = decisionMessages(run, engine.models).filter(message => message.phase.startsWith("主持解读"))
    expect(interpretations).toHaveLength(run.round)
    expect(interpretations[0]?.text).toContain("建议下一步")
